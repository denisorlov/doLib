...
//            outerDiv.appendChild(headTable);
        ...
        /**
         * @param columnIndex индекс колонки от 0, для связи с размером колонки через css и события, nullable
         * @param content
         * @param TH boolean &lt;td&gt; or &lt;th&gt;
         * @returns {HTMLElement} &lt;td&gt;(or &lt;th&gt;)&lt;div&gt;[content]&lt;/div&gt;&lt;/td&gt;
         */
        this.createCell = function(columnIndex, content, TH){
            ...
            if(columnIndex!=null){
                div.style.width = calcColumnWidth(columnIndex)+'px';
                div.setAttribute(columnIndexAttrName, columnIndex);
            }
        ...
        this.createCellTH = function(index, content, groupId, resize, drag){
            var th = this.createCell(index, content, true),
                div = getCellDiv(th),
                _this = this;
        ...
        this.headAppendRow = function(tr){
            var head = getTHead(), i,
	...
	{// private functions
        ...
        initDragger = function(){
            ...
            thDragger.options.onStart = function(event, th){
                var thArr = getSameGroupTh(th);
                for(var i in thArr){
                    var ind1 = getCellDiv(thArr[i]).getAttribute(columnIndexAttrName),
                        ind2 = getCellDiv(th).getAttribute(columnIndexAttrName);
                    if(ind1==ind2){// та, что тащим
                        thArr[i].classList.add(dragThSourceClassName);
                    }else{// куда тащим
                        thArr[i].classList.add(dragThTargetClassName);
                    }
                }
            ...
            thDragger.options.onEnd = function(event, th){
                var pageX = event.pageX,
                    thArr = getSameGroupTh(th);
                for(var i in thArr){
                    thArr[i].classList.remove(dragThSourceClassName);
                    thArr[i].classList.remove(dragThTargetClassName);
                    var coords = $do.elementUtils.getCoords(thArr[i]);
                    if(coords.left<=pageX && coords.right>=pageX){
                        moveColumn(getCellDiv(th).getAttribute(columnIndexAttrName),
                            getCellDiv(thArr[i]).getAttribute(columnIndexAttrName));
                    }
                }
        ...
        initResizer = function(){
            ...
            thResizer.options.onResize = function(event, element){
                var colIndex = element.getAttribute(columnIndexAttrName);
                if(colIndex) _this.setColumnWidth(colIndex, element.style.width);
...
//		},
        ...
        /** div контейнер содержимого ячейки */
        getCellDiv = function(th){
            return th.querySelector('.'+cellClassName);
        },

        getSameGroupTh = function(th){
            var res=[],
                groupId = th.getAttribute(groupIdAttrName),
                level = th.getAttribute(headLevelAttrName),
                c = document.querySelectorAll('#'+outerDiv.id+
                    ' tr['+headLevelAttrName+'="'+level+'"]  th['+groupIdAttrName+'="'+groupId+'"]');
            for(i in c)
                if(c[i] instanceof Node)
                    res.push(c[i]);
            return res;
        }
		;

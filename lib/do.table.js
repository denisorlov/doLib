...
$do.table.Options = function(obj){
    ...
    /** получение и вывод данных, условно абстрактный метод, требуется определить */
    this.getAndOutData = null;
    /** создание массива строк TR данных, условно абстрактный метод, требуется определить */
...
$do.table.Object = function(tableId, _options){
    ...
    var _this = this,// for private methods
        ...
        plateLeftPercent = 30, // left плашки в процентах
...
//            return tr;
//        }
        /** Вывод строк, метод должен быть вызван из метода {@link getAndOutData} */
        this.outRows = function(data){
            var i, orderPlanObj,
                rows = _this.createRowsArray(data),
                rowIndex = lastRowStart,
                fragment = document.createDocumentFragment();
            for(i in rows){
                setColumnIndexAttrs(rows[i].querySelectorAll('td'));
                rows[i].setAttribute(rowIndexAttrName, rowIndex++);
                fragment.appendChild(rows[i]);
                if(i==0){
                    var headMap = getColumnIndexMap(getTHead().querySelectorAll('th['+columnIndexAttrName+']'));
                    orderPlanObj = getOrderCellsPlan(rows[i], headMap);
                }else{
                    orderCellsByPlan(rows[i], orderPlanObj);
                }
            }
...
//            }
            ...
            if(scrollToAfterOutRows>-1){
                _this.scrollTo(scrollToAfterOutRows); clog('scrollTo '+scrollToAfterOutRows);
         ...
         * метод должен быть вызван из метода {@link getAndOutData}
         */
        ...
        this.init = function(){
            ...
            var _template =
                ...
                '    <div id=">tableId<_mainDiv" style="overflow: hidden;padding-bottom: 100px;/*for table bottom border*/">'+
                ...
                '    <div class="'+plateClassName+'" style="left: '+plateLeftPercent+'%;"></div>'+
                '</div>',
            ...
            $do.eventUtils.addEvent(outerDiv, 'scroll', (function(){//closure
                return function(){
                    var outerDivW = parseInt($do.elementUtils.getStyle(outerDiv).width);
                    scrollBarDiv.style.right = -this.scrollLeft+'px';
                    shield.style.right = -this.scrollLeft+'px';
                    plate.style.left = this.scrollLeft?(this.scrollLeft+Math.floor(outerDivW*plateLeftPercent/100))+'px':plateLeftPercent+'%';
                }
            }())
            );
            table.parentNode.insertBefore(outerDiv, table);
//            var fixSize = function(){_this.fixSize()};
...
//            $do.eventUtils.addEvent(window, 'resize', fixSize);
        ...
        this.createCell = function(content, TH){
            ...
            content instanceof Node ? div.appendChild(content) : div.innerHTML = content;
            div.title = div.textContent.replace(/"/g, "'");
        ...
        /** общая подгонка размеров TODO разобрать */
    ...
    /** вывести/спрятать плашку с сообщением content */
    this.plateToggle = function(content){
        plate.innerHTML = '';
        content instanceof Node ? plate.appendChild(content) : plate.innerHTML = content;
    ...
    {// accessors
        ...
        this.setPlateLeftPercent = function(plate_left_percent){
            plateLeftPercent = parseInt(plate_left_percent);
        }
        ...
        this.getSelectedRows = function(){
            return this.getTBody().querySelectorAll('tr.'+clickedRowClassName);
        }
	...
	{// private functions
        ...
        /**
         * создать план перемещения ячеек(TD) согласно заголовкам
         * на примере строки row и карты позиций заголовков headMap
         * see {@link getColumnIndexMap()} */
        getOrderCellsPlan = function(row, headMap){
            var planArr = [],
                nodeList = row.querySelectorAll('td['+columnIndexAttrName+']'),
                rowMap = getColumnIndexMap(nodeList),
                orderMap = getOrderMap(nodeList);//
            for(var o in headMap){
                if(headMap[o]!=rowMap[o]){
                    //console.dir(orderMap); console.dir(headMap); console.dir(rowMap);console.log(orderMap[headMap[o]],' --> ',o);
                    moveCell(row, orderMap[headMap[o]], o);// позиция индекса в позицию заголовка
                    planArr.push({sPos: orderMap[headMap[o]], tPos: o});// заполняем план перестановки ячеек
                }
                nodeList = row.querySelectorAll('td['+columnIndexAttrName+']');
                rowMap = getColumnIndexMap(nodeList);
                orderMap = getOrderMap(nodeList);
        ...
        /** переместить ячейки(TD) строки row согласно плану planArr, see {@link getOrderCellsPlan()}  */
        orderCellsByPlan = function(row, planArr){
            for(var i=0;i<planArr.length;i++){
                var plan = planArr[i];
                moveCell(row, plan.sPos, plan.tPos);
            }
        },
        ...
        checkInit = function(){
            if(typeof _this.getAndOutData != 'function'){
                alert("Для объекта $do.table не определен метод \n" +
                    ...
                    " this.setTotalRowCount( общее_количество_строк ) - для построения скролл-бара\n"+
        ...
        initTable = function(){
            $do.eventUtils.addEvent(table, 'click',
                function(e){
                    ...
                    if(!e.ctrlKey && selectedRow!=null) selectedRow.classList.remove(clickedRowClassName);// TODO selectedRowList ?  shiftKey, ctrlKey
                    selectedRow = tr;

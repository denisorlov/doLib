...
//            outerDiv.appendChild(headTable);
        ...
        this.createCellTH = function(index, content, groupId, resize, drag){
            ...
            if(drag){
                /*
                 clone = $do.elementUtils.cloneNode(document.querySelectorAll("[data-group-id='3']")[0], true);
                 clone.style.position = 'absolute';
                 clone.style.top= '200px'; clone.style.left= '300px';
                 clone.style.backgroundColor = 'white';
                 document.body.appendChild(clone)
                */
                var onDrag = thDragger.options.onDrag;
                thDragger.options.onDrag = function(event, element){
                    

                    if(typeof onDrag == 'function'){
                        onDrag.apply(thDragger, arguments);
                    }
                };
